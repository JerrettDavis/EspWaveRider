    ...
    T: Read + Write,
{
    ...
    /// The last transport error observed inside the BIO callbacks.
    io_error: Option<ErrorKind>,
    /// The last BIO activity observed during an MbedTLS call.
    debug: SessionIoDebug,
    /// Reference to the active Tls instance
    _token: TlsReference<'a>,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
struct SessionIoDebug {
    last_mbedtls_result: c_int,
    last_bio_receive_requested: usize,
    last_bio_receive_result: c_int,
    last_bio_receive_preview_len: usize,
    last_bio_receive_preview: [u8; 8],
    last_bio_send_requested: usize,
    last_bio_send_result: c_int,
    last_wait_readable_result: c_int,
    last_wait_writable_result: c_int,
}

impl SessionIoDebug {
    const UNSET_RESULT: c_int = i32::MIN;
    const UNSET_REQUESTED: usize = usize::MAX;

    const fn new() -> Self {
        Self {
            last_mbedtls_result: Self::UNSET_RESULT,
            last_bio_receive_requested: Self::UNSET_REQUESTED,
            last_bio_receive_result: Self::UNSET_RESULT,
            last_bio_receive_preview_len: 0,
            last_bio_receive_preview: [0; 8],
            last_bio_send_requested: Self::UNSET_REQUESTED,
            last_bio_send_result: Self::UNSET_RESULT,
            last_wait_readable_result: Self::UNSET_RESULT,
            last_wait_writable_result: Self::UNSET_RESULT,
        }
    }
...
pub struct SessionDebugStatus {
    pub state: c_int,
    pub handshake_over: bool,
    pub pending: bool,
    pub bytes_avail: usize,
    pub eof: bool,
    pub buffered_read_byte: bool,
    pub buffered_write_byte: bool,
    pub last_mbedtls_result: c_int,
    pub last_bio_receive_requested: Option<usize>,
    pub last_bio_receive_result: Option<c_int>,
    pub last_bio_receive_preview_len: usize,
    pub last_bio_receive_preview: [u8; 8],
    pub last_bio_send_requested: Option<usize>,
    pub last_bio_send_result: Option<c_int>,
    pub last_wait_readable_result: Option<c_int>,
    pub last_wait_writable_result: Option<c_int>,
}
    ...
    T: Read + Write,
{
    ...
    pub fn debug_status(&self) -> SessionDebugStatus {
        let state = self.state.ssl_context.private_state;

        SessionDebugStatus {
            state,
            handshake_over: state == mbedtls_ssl_states_MBEDTLS_SSL_HANDSHAKE_OVER as c_int,
            pending: unsafe { mbedtls_ssl_check_pending(&*self.state.ssl_context) != 0 },
            bytes_avail: unsafe { mbedtls_ssl_get_bytes_avail(&*self.state.ssl_context) },
            eof: self.eof,
            buffered_read_byte: self.read_byte.is_some(),
            buffered_write_byte: self.write_byte.is_some(),
            last_mbedtls_result: self.debug.last_mbedtls_result,
            last_bio_receive_requested: (self.debug.last_bio_receive_requested
                != SessionIoDebug::UNSET_REQUESTED)
                .then_some(self.debug.last_bio_receive_requested),
            last_bio_receive_result: (self.debug.last_bio_receive_result
                != SessionIoDebug::UNSET_RESULT)
                .then_some(self.debug.last_bio_receive_result),
            last_bio_receive_preview_len: self.debug.last_bio_receive_preview_len,
            last_bio_receive_preview: self.debug.last_bio_receive_preview,
            last_bio_send_requested: (self.debug.last_bio_send_requested
                != SessionIoDebug::UNSET_REQUESTED)
                .then_some(self.debug.last_bio_send_requested),
            last_bio_send_result: (self.debug.last_bio_send_result
                != SessionIoDebug::UNSET_RESULT)
                .then_some(self.debug.last_bio_send_result),
            last_wait_readable_result: (self.debug.last_wait_readable_result
                != SessionIoDebug::UNSET_RESULT)
                .then_some(self.debug.last_wait_readable_result),
            last_wait_writable_result: (self.debug.last_wait_writable_result
                != SessionIoDebug::UNSET_RESULT)
                .then_some(self.debug.last_wait_writable_result),
        }
    }
...
struct MBio<'a, T> {
    ...
    write_byte: &'a mut Option<u8>,
    /// The last transport error observed inside the BIO callbacks.
    io_error: &'a mut Option<ErrorKind>,
    /// The last BIO activity observed during an MbedTLS call.
    debug: &'a mut SessionIoDebug,
}
    ...
    T: Read + Write,
{
    ...
    fn take_io_error(&mut self) -> Option<SessionError> {
        self.io_error.take().map(SessionError::from)
    }
    ...
    async fn wait_readable(&mut self) -> Result<bool, T::Error> {
        if self.read_byte.is_none() {
            ...
            if len == 0 {
                self.debug.last_wait_readable_result = 0;
                return Ok(false);
            }
        ...
        self.debug.last_wait_readable_result = 1;
    ...
    async fn wait_writable(&mut self) -> Result<bool, T::Error> {
        if let Some(byte) = self.write_byte.as_ref() {
            ...
            if len == 0 {
                self.debug.last_wait_writable_result = 0;
                return Ok(false);
            }
        ...
        self.debug.last_wait_writable_result = 1;
        ...
        F: FnMut(&mbedtls_ssl_context) -> i32,
    {
        ...
        poll_fn(|ctx| {
            ...
            io_ctx.io.debug.last_mbedtls_result = result;
    ...
    fn bio_receive(&mut self, buf: &mut [u8], ctx: &mut Context<'_>) -> i32 {
        ...
        self.debug.last_bio_receive_requested = buf.len();

        let result = match self.poll_read(ctx, buf) {
            Poll::Ready(len) => len as _,
            Poll::Pending => MBEDTLS_ERR_SSL_WANT_READ,
        };

        self.debug.last_bio_receive_result = result;
        self.debug.last_bio_receive_preview_len = 0;
        self.debug.last_bio_receive_preview.fill(0);
        if result > 0 {
            let preview_len = core::cmp::min(result as usize, self.debug.last_bio_receive_preview.len());
            self.debug.last_bio_receive_preview[..preview_len].copy_from_slice(&buf[..preview_len]);
            self.debug.last_bio_receive_preview_len = preview_len;
        }
        result
    }
    ...
    fn bio_send(&mut self, buf: &[u8], ctx: &mut Context<'_>) -> i32 {
        ...
        self.debug.last_bio_send_requested = buf.len();

        let result = match self.poll_write(ctx, buf) {
            Poll::Ready(len) => len as _,
            Poll::Pending => MBEDTLS_ERR_SSL_WANT_WRITE,
        };

        self.debug.last_bio_send_result = result;
        result
    }
