...
- Rust node `10.0.107.148`: `mqtt_connected=true`, `udp_discovery.peer_count=1`, `room_peers=0`
- C++ node `10.0.107.149`: `mqtt_connected=true`, `udp_discovery.peer_count=0`, `room_peers=1`

That means transport parity is substantially restored, but cross-node collaboration is still asymmetric in live snapshots and should be treated as `Partial` until the Rust node consistently reports the C++ room peer and both nodes consistently report each other over UDP discovery.
...
| `ha_room_pose_publish:` | Publishes pose command/event | Parses command but returns snapshot | Partial | Code read only | No pose publish parity yet |
...
| Room peer snapshot | MQTT-backed room collaboration peers | MQTT-backed room collaboration peers | Partial | Live hardware | C++ sees Rust; Rust intermittently reports zero peers |
...
| Room summary subscribe | Active | Active but asymmetric | Partial | Live hardware + new host tests | Payload parsing is covered; live peer visibility still flaps |
| UDP discovery announce/receive | Active | Active but asymmetric | Partial | Live hardware | Rust currently sees C++; C++ currently does not reliably retain Rust UDP peer |
...
| Live multi-node room fusion | Active | Restored but asymmetric | Partial | C++ sees Rust consistently; Rust room peer view still flaps |
| Live multi-node UDP discovery | Active | Restored but asymmetric | Partial | Rust reports one UDP peer; C++ currently reports zero |

## Highest Priority Gaps

1. Stabilize Rust inbound `room_peers` so the Rust node consistently reports the C++ node in live snapshots.
2. Stabilize UDP discovery symmetry so both nodes retain each other in `udp_discovery.peers` at the same time.
3. Implement actual OTA download/apply behavior behind `firmware_sync` / `firmware_update`.
4. Implement BLE beacon/tag observation and command handling instead of placeholder zero/empty snapshot fields.
5. Implement real `wifi_scan` and `ha_room_pose_publish` command behavior in Rust.
